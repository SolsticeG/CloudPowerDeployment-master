...
public class DatacenterBroker extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource characteristics request
			case CloudSimTags.RESOURCE_CHARACTERISTICS_REQUEST:
				System.out.println("RESOURCE_CHARACTERISTICS_REQUEST");
				processResourceCharacteristicsRequest(ev);
				break;
			// Resource characteristics answer
			case CloudSimTags.RESOURCE_CHARACTERISTICS:
				System.out.println("RESOURCE_CHARACTERISTICS");
				processResourceCharacteristics(ev);
				break;
			// VM Creation answer
			case CloudSimTags.VM_CREATE_ACK:
				System.out.println("VM_CREATE_ACK");
				processVmCreate(ev);
				break;
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		Log.printConcatLine(CloudSim.clock(), ": ", getName(), ": Cloudlet ", cloudlet.getCloudletId(),
				" received");
		//Deallocate vm when cloudlet returns
		
		int bindVmId = cloudlet.getVmId();
		Vm bindVm = getVmList().get(bindVmId);
		Host bindHost = bindVm.getHost();
		bindHost.vmDestroy(bindVm);
		...
		if (getCloudletList().size() == 0 && cloudletsSubmitted == 0) { // all cloudlets executed
			...
			if (getCloudletList().size() > 0 && cloudletsSubmitted == 0) {
				...
				System.out.println("some cloudlets haven't finished yet");
				clearDatacenters();
