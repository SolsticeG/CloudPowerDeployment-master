...
import static newcloud.ExceuteData.FairScheduleTest.brokerId;
...
public class PowerDatacenterFair extends PowerDatacenter {
    ...
    protected void processVmCreate(SimEvent ev, boolean ack) {    	
    	System.out.println("processVmCreate in Fair");
    	Vm vm = (Vm) ev.getData();
    	//vm requested resources
    	List<Double> vmStat = new ArrayList<>();
    	double requestedMaxMips = vm.getCurrentRequestedMaxMips();
    	double requestedTotalMips = vm.getCurrentRequestedTotalMips();
    	double requestedRam = vm.getCurrentRequestedRam();
    	double requestedBw = vm.getCurrentRequestedBw();
    	vmStat.add(requestedMaxMips);
    	vmStat.add(requestedTotalMips);
    	vmStat.add(requestedRam);
    	vmStat.add(requestedBw);
    	/*
    	System.out.println(requestedMaxMips);
    	System.out.println(requestedTotalMips);
    	System.out.println(requestedRam);
    	System.out.println(requestedBw);
    	*/
    	
    	//Host Status
    	List<PowerHost> hostList = getHostList();
    	List<Double> hostInfo = new ArrayList<>();
    	//double[] hostInfo = new double[5];
    	for(int i=0;i < hostList.size();i++) {
    		PowerHost host = hostList.get(i);
    		double hostMaxMips = host.getVmScheduler().getPeCapacity();
    		double hostAvailableMips = host.getVmScheduler().getAvailableMips();
    		double hostAvailableRam = host.getRamProvisioner().getAvailableRam();
    		double hostAvailableBw = host.getBwProvisioner().getAvailableBw();
    		double hostCPUUtilization = host.getUtilizationOfCpu() * 100;
    		
    		hostInfo.add(hostMaxMips);
    		hostInfo.add(hostAvailableMips);
    		hostInfo.add(hostAvailableRam);
    		hostInfo.add(hostAvailableBw);
    		hostInfo.add(hostCPUUtilization);
    		/*
    		System.out.println(hostMaxMips);
        	System.out.println(hostAvailableMips);
        	System.out.println(hostAvailableRam);
        	System.out.println(hostAvailableBw);
        	System.out.println(hostCPUUtilization);
        	*/
    	}
    	
    	Process proc;
    	//String infoType = "VmHost";
    	try {
    		/*
    		String[] args = new String[] {"python","C:\\Users\\gxq\\Desktop\\CloudPowerDeployment-master\\src\\main\\java\\newcloud\\policy\\pythonPolicy.py",String.valueOf(hostInfo),String.valueOf(requestedMaxMips),
    				String.valueOf(requestedTotalMips),String.valueOf(requestedRam),String.valueOf(requestedBw),
    		};
    		*/
    		String command = "python ";
    		String path = "C:\\\\Users\\\\gxq\\\\Desktop\\\\CloudPowerDeployment-master\\\\src\\\\main\\\\java\\\\newcloud\\\\policy\\\\pythonPolicy.py ";
    		proc = Runtime.getRuntime().exec(command + path + hostInfo + vmStat);
        ...
        targetHost = vmAllocationAssignerFair.getVmAllocationHost(getHostList());
        ...
        if (ack) {
            ...
            System.out.println("update cloudlet processing");
...
//        }
        ...
        if (currentTime > getLastProcessTime()) {
            ...
            if (minTime != Double.MAX_VALUE) {
            	System.out.println("Schedules an event to the next time");
                boolean flag = CloudSim.cancelAll(getId(), new PredicateType(CloudSimTags.VM_DATACENTER_EVENT));
                System.out.println(flag);
                System.out.println("send VM_DATACENTER_EVENT");
    ...
    protected double updateCloudetProcessingWithoutSchedulingFutureEventsForce() {
        ...
        System.out.println("check cloudlet completion");
...
//        }
    ...
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
        System.out.println("processCloudletSubmit in DatacenterFair");
    	updateCloudletProcessing();
