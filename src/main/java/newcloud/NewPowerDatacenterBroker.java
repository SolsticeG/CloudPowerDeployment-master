...
public class NewPowerDatacenterBroker extends PowerDatacenterBroker {
    ...
    protected void processOtherEvent(SimEvent ev) {
        switch (ev.getTag()) {
            case CREATE_VM_ACK:
            	System.out.println("CREATE_VM_ACK");
                processVmCreate(ev);
                break;
    ...
    protected void processVmCreate(SimEvent ev) {
        ...
        if (getVmsCreatedList().size() > 0 && getVmsCreatedList().size() <= getVmList().size() && flag == 1) {
        	System.out.println("submit cloudlet");
            submitCloudlets();
        } else {
        	
            // all the acks received, but some VMs were not created
            if (getVmsRequested() == getVmsAcks()) {
            	System.out.println("find id of the next datacenter that has not been tried");
    ...
    protected void createVmsInDatacenter(int datacenterId) {
        ...
        //boolean flag1 = !getVmsToDatacentersMap().containsKey(vm.getId());
        //boolean flag2 = requestedVms < getVmList().size() - 1;
